...
```{r load_merge,cache=TRUE,echo = FALSE}
...
```{r clean_odk,cache=TRUE,echo = FALSE}
...
all_merged = merge(pa_data, #data every other minute
  ...
  dplyr::mutate(N = n(),
                ...
                region = case_when(filename.x %like% "Dagoretti" ~ "Dagoretti",
                                   TRUE ~ "Starehe"),
                exposure_category = case_when(pm25_larpa_ave <= 10 ~ "Exposure<10",
                                              pm25_larpa_ave > 10 & pm25_larpa_ave < 35 ~ "10<Exposure<35",
                                              pm25_larpa_ave > 35 & pm25_larpa_ave < 100~ "Exposure>35",
                                              TRUE ~ "Exposure>100"),
...
summary_pa_deployment <- all_merged %>% 
  ...
  dplyr::left_join(
    ...
    by = "hhid")
DT::datatable(summary_pa_deployment, caption = "PurpleAir Summary Table","pipe")
...
g_timeofday_pa <- all_merged %>% 
  ...
  ggplot(aes(x=time_of_day, y = pm25_larpa_ave,color = sampletype)) +
...
g_categories_bars_exposure <- all_merged %>% 
  ggplot(aes(x = hhid,fill=exposure_category, y = pm25_larpa_ave)) +
  geom_bar(position="fill", stat="identity") + 
...
g_categories_bars_exposure <- all_merged %>% 
  dplyr::mutate(time_of_daystr = strftime(local_time, format="%H:%M:%S",tz = local_tz),
                time_of_day = as.POSIXct(time_of_daystr, format="%H",tz = local_tz))  %>% 
