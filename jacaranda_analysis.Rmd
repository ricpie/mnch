...
reimport = TRUE #Set to true if we have new GPS or PA data.
...
```{r load_merge,cache=TRUE,echo = FALSE,include=FALSE}
...
odkend_time_activities <- read_xlsx("~/Dropbox/Jacaranda Kenya/Data Analysis mnch/Results/processed data/odkend_time_activities_processed_mm_7-7-21.xlsx") %>% 
  ...
  dplyr::mutate(activity_clean = 
                  case_when(activity %like% "Visit|visit|teacher|friend|Greet|neighbo|relative|Shopping" ~ "Errands_out",
                            activity %like% "Cyber|cyber|Repair|walk" ~ "Errands_out",
                            ...
                            activity %like% "walk|neighborhood" ~ "Errands_out",
                            activity %like% "Bank|Posho|report|Pay_rent|Meeting|Gh|Fetch|Chamas|bank" ~ "Errands_out",
                            TRUE ~ activity))
...
all_merged = merge(pa_data, #data every other minute
  ...
  dplyr::left_join(odkstart %>% 
                     ...
                     dplyr::rename(pa_id = BAmbientStartB4PurpleAirID,
                                   hhid_ambient = hhid),
                   by = "pa_id") %>% 
  dplyr::mutate(hhid = case_when(sampletype == "ambient" & !is.na(BAmbientStartB2location) ~ hhid_ambient,
                                 TRUE ~ hhid),
...
DT::datatable(summary_overall, caption = "PurpleAir Overall Summary Table")
...
summary_by_activity <- all_merged %>% 
  ...
  dplyr::mutate(activity_clean = 
                  case_when(is.na(activity_clean) ~ "Errands_home",
                            activity_clean %like% "Working|Errands|Shopping|Bank|Visiting" ~ "Errands_out",
                            activity_clean %like% "Chores|Childcare|Resting|Sleeping" ~ "Errands_home",
                ...
                activity_clean = 
                  factor(activity_clean,
                         levels = c("Wood_stove", "Charcoal_stove",  
                                    ...
                                    "Chores","Working","Errands_out","Errands_home","Shopping","Bank","Childcare","Resting",
...
g_timeofday_pa <- all_merged %>% 
  ...
  ggplot(aes(x=time_of_day, y = pm25_bam_ave,color = sampletype)) +
  ...
  facet_grid(region~.) + 
  theme(axis.text.x=element_text(angle=45,hjust= 1 ,vjust = 1)) +
  geom_text(aes(min(time_of_day),11,label = "             WHO Guideline (10 µg/m3)",vjust = 2)) +
  geom_hline(aes(yintercept=35)) +
  geom_text(aes(min(time_of_day),36,label = "             WHO Interim-1 Target (35 µg/m3)",vjust = 2))
g_timeofday_pa
...
g_boxplot <- ggplot(pa_ambient,aes(x=hhid, y = pm25_bam_ave)) +
  ...
  stat_summary(fun.data = give.mean5, geom = "text",colour="blue") +
  theme(axis.text.x=element_text(angle=45,hjust= 1 ,vjust = 1)) +
  theme(legend.title = element_blank())
g_boxplot
...
modeldata_continuous <- modeldata %>%  dplyr::select(Mean_pm25 ,tobacco_exp_freq,
                                                     ...
                                                     tobacco_exp_freq,Charcoal_stove,Cooking,#D8_lightingtype
                                                     Errands_home,Errands_out,Incense,Kerosene_stove,   LPG_stove,#Mosquito_coil,       
                                                     Wood_stove,Working) %>% 
